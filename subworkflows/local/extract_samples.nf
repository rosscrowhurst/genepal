...
//
// Check input fastqsheet and get read channels
//
// Changes:
...
include { FASTQSHEET_CHECK } from '../../modules/local/fastqsheet_check'

workflow EXTRACT_SAMPLES {
    take:
    fastqsheet                      // file: /path/to/fastqsheet.csv
    permissible_target_assemblies   // val: assembly_a,assembly_b
    exclude_assemblies              // channel: val(assembly_x,assembly_y)

    main:
    FASTQSHEET_CHECK ( fastqsheet, permissible_target_assemblies )
    .csv
    ...
    versions = FASTQSHEET_CHECK.out.versions    // channel: [ versions.yml ]
}
...
def create_fastq_channel(LinkedHashMap row, sheetPath) {
    ...
    def fastq_meta = []
    if (!file(fq1).exists()) {
        exit 1, "ERROR: Please check input fastqsheet -> Read 1 FastQ file does not exist!\n${fq1}"
    }
    if (meta.single_end) {
        fastq_meta = [ meta, [ file(fq1) ] ]
    } else {
        if (!file(fq2).exists()) {
            exit 1, "ERROR: Please check input fastqsheet -> Read 2 FastQ file does not exist!\n${fq2}"
        }
